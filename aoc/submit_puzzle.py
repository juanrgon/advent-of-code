...
def submit_puzzle(*, script_filename: str, part: int, solution):
    ...
    try:
        ...
        raise RuntimeError(f"Failed to submit puzzle for {script.year} Day {script.day} Part {part}") from e

    result = "\n" + HTML(html=response.text).find("article")[0].full_text

    if "You don't seem to be solving the right level.  Did you already complete it?" in result:
        return
