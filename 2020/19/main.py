...
y = """bbabbaaaabbaabbbaabbabaabbaabbaababaabab
...
@lru_cache
def factor(rule):
    if rule == re.fullmatch("^[ab]*$", rule):
        return rule

    parenthesised = re.fullmatch(r'^\((.*)\)$', rule)
    if parenthesised:
        rule = factor(parenthesised.group(1))


    return '|'.join([factor(rules[index]) for index in rules])


print(factor(rules["0"]))
